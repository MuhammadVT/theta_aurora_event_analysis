...
def plot_max_point_tracks(stm, etm, read_si=True, read_wic=False,
...
#    #image_data = imfilter(image_data, 'edge_enhance')
#    #image_data = image_data - 200
#    cax1 = ax1.imshow(image_data, vmin=200)
#    ax1.set_axis_off()
#    ax1.set_title(dtm.strftime("%Y/%m/%d %H:%M") + ss_txt)
#    cbar1 = fig1.colorbar(cax1)

    #fig, ax = plt.subplots()
    #edge = feature.canny(data['image'], sigma=5)
    #ax.imshow(edge)
    #ax.set_axis_off()
