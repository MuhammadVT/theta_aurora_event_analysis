...
stm = dt.datetime(2002,3,18,15,30)
...
dtms, fnames = find_filenames(stm, etm, read_si=True, read_wic=False)
...
for dtm in dtms:
    ...
    if wic_si_ratio:
        ...
        fname = "normalized_wic_si_ratio_" + dtm.strftime("%H%M")
        ...
        if wic_only:
            ...
            fname = "wic_overlayed_" + dtm.strftime("%H%M")
            fdir ="../plots/multiinstrument_data_overlay/WIC/"
        if si_only:
            IMAGE_si = IMAGE_sat(dtm, datatype="SI")
            IMAGE_si.overlay_data(mobj, param="image",
                                   interpolate_data=interpolate_IMAGE_data,
            ...
            fname = "si_overlayed_" + dtm.strftime("%H%M")
            fdir ="../plots/multiinstrument_data_overlay/SI/"
