class IMAGE_sat():
    ...
    def overlay_data(self, mobj, param="image",
        ...
        if interpolate_data:
            ...
            Xtmp = X; Ytmp = Y
            Xtmp[np.where(Xtmp > 100*111.e3)] = 0*111.e3
            ...
            mappable = mobj.pcolor(Xnew, Ynew, Znew, vmin=vmin, vmax=vmax, cmap=cmap)
        else:
            mobj.pcolormesh(X, Y, Z, vmin=vmin, vmax=vmax, cmap=cmap)
            if gauss_filter:
                Z = gaussian_filter(Z, 1)
            mappable = mobj.pcolor(X, Y, Z, vmin=vmin, vmax=vmax, cmap=cmap)
        ...
        if mark_max_intensity_point:
            if param == "image": 
                ...
                maxloc_X = X[maxloc[0], maxloc[1]]
                maxloc_Y = Y[maxloc[0], maxloc[1]]
                mobj.scatter(maxloc_X, maxloc_Y, color='r', s=2)
            else:
                pass
